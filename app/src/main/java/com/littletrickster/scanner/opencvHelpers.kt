...
fun autoBrightnessGray(gray: Mat): Mat {
    ...
    val width = gray.width() * 2
    val height = gray.height() * 2
    val scaledOriginal = Mat()
    Imgproc.resize(gray, scaledOriginal, Size(width.toDouble(), height.toDouble()))
    val (alpha, beta) = getAutoBrightnessAlphaAndBetaFromGray(scaledOriginal)
    val final = Mat()
    Core.convertScaleAbs(scaledOriginal, final, alpha, beta)
    return final
}
...
fun documentMat(original: Mat): Mat {
    ...
    val scaledOriginal = Mat()
    Imgproc.resize(original, scaledOriginal, Size(width.toDouble(), height.toDouble()))

    val blurred = Mat()
    Imgproc.blur(scaledOriginal, blurred, Size(5.0, 5.0))
...
//    Core.max(result, maskInverted, finalInvertedMask)
    ...
    val gray = Mat()
    Imgproc.cvtColor(scaledOriginal, gray, Imgproc.COLOR_BGR2GRAY)
    val text = Mat()
...
//    Core.multiply(mask,Scalar(1.2),mask)
    ...
    val colored = Mat()
    Core.bitwise_and(scaledOriginal, scaledOriginal, colored, mask)
    val bwStuff = Mat()
